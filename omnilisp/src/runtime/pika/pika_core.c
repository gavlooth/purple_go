...
static PikaMatch evaluate_rule(PikaState* state, size_t pos, int rule_id) {
    ...
    switch (rule->type) {
        case PIKA_TERMINAL: {
            ...
            if (pos + len <= state->input_len && strncmp(state->input + pos, rule->data.str, len) == 0) {
                m.matched = true;
                m.len = len;
            }
            break;
        ...
        case PIKA_REP: {
            // Zero or more: A*
            // Equivalent to (A A*) / epsilon
            PikaMatch* first = get_match(state, pos, rule->data.children.subrules[0]);
            if (first && first->matched && first->len > 0) {
                // Recursive step: look for A* at pos + len
                // Since we compute Right-to-Left, the recursive result for this rule at (pos+len) is already computed!
                // Wait, not necessarily. Pika computes by position index.
                // If we iterate pos from End to Start, then (pos + len) > pos, so it IS computed.
                PikaMatch* rest = get_match(state, pos + first->len, rule_id);
                if (rest && rest->matched) {
                    m.matched = true;
                    m.len = first->len + rest->len;
                } else {
                    // Should be impossible if REP matches epsilon at end, but strictly:
                    m = *first; 
                }
            } else {
                // Match empty
...
Value* pika_run(PikaState* state, int root_rule_id) {
    ...
    if (root && root->matched) {
        // For now, return the matched string as a Value if no action
        if (root->val) return root->val;
        char* s = malloc(root->len + 1);
        memcpy(s, state->input, root->len);
        s[root->len] = '\0';
        Value* v = mk_sym(s); // Should ideally be mk_string but symbols work for testing
        free(s);
        return v;
    }
    
    return mk_error("Pika: Match failed");
}